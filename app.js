...
(function () {
      ...
      const body = g.items
        .map((item) => {
          ...
          return `<label class="ctrl">
            <div class="ctrl-top">
              <span>${item.label}${item.homework ? ' <em>homework</em>' : ""}</span>
              <span class="ctrl-val">
                <input id="n-${item.id}" type="number" min="${item.min}" max="${item.max}" step="${item.step}" value="${v}" />
                <abbr>${item.unit}</abbr>
              </span>
            </div>
            <input id="r-${item.id}" type="range" min="${item.min}" max="${item.max}" step="${item.step}" value="${v}" />
          </label>`;
        })
        .join("");
  ...
  function tick() {
    renderResults(state, compute(state));
  }
